...
const StationsMap = () => {
  ...
  // Pobierz stacje z współrzędnymi
    ...
    try {
      ...
      if (data.status === 'success') {
        console.log('Received stations data:', {
          ...
          sample: data.stations.slice(0, 3),
          alarmStats: {
            normal: data.stations.filter((s: StationData) => s.alarmStatus === 'normal').length,
            warning: data.stations.filter((s: StationData) => s.alarmStatus === 'warning').length,
            alarm: data.stations.filter((s: StationData) => s.alarmStatus === 'alarm').length,
            unknown: data.stations.filter((s: StationData) => s.alarmStatus === 'unknown').length
          }
        });
        setStations(data.stations);
      } else {
        setError(data.error || 'Błąd podczas pobierania stacji');
      }
    } catch (err) {
